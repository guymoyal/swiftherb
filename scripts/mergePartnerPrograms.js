#!/usr/bin/env node
/**
 * Merge harvested partner-network programs (content/partner-programs.json)
 * into content/admitad-landings.json as landing entries (fetchMode: "manual").
 *
 * Admitad's "Affiliate programs from partners" tab (tatrck.com links, no advertiser
 * approval needed) is not exposed by the publisher API, so the harvester writes
 * partner-programs.json and this script turns it into landing entries. Copy
 * generation and page rendering treat these entries like API ones.
 *
 ...
 * matching the harvester's retention behavior.
 *
 * Usage: pnpm partners:manual  (also part of pnpm partners:sync)
 */
const fs = require('fs');
...
function reservedRouteSegments() {
  ...
  // Not app/ segments, but served from the same root namespace.
  reserved.add('partners');
  reserved.add('images');
  return reserved;
}
