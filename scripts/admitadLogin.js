 ...
 *
 * Note: the harvester can also reuse an existing logged-in profile from another repo
 * via ADMITAD_PROFILE_DIR (see scripts/harvestPartnerPrograms.js), so running this
 * script is only needed when no valid session exists anywhere.
 *
 ...
 *
 * Usage: pnpm partners:login
 */
const fs = require('fs');
const path = require('path');
const { chromium } = require('playwright');

const WEBSITE_ID = process.env.ADMIT_WEBSITE_ID || '2913701';
const CATALOG_URL = `https://store.admitad.com/en/webmaster/websites/${WEBSITE_ID}/catalog/partners_programs/`;
const PROFILE_DIR = process.env.ADMITAD_PROFILE_DIR || path.join(__dirname, '..', '.admitad-profile');
...
(async () => {
  ...
  console.log(`Session saved in ${PROFILE_DIR}; ${netlog.length} JSON responses logged.`);
